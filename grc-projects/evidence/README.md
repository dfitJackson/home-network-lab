
🏡 [Home](../../README.md) • ⬅️ [GRC Projects](../README.md) 

# 🗂️ Evidence Folder

This folder contains supporting files, screenshots, export data, PDFs, and other artifacts related to GRC projects.

Use this folder to store any documentation produced during:
- ISO 27001 gap analysis
- Control mappings
- Policies and procedures
- Risk assessments
- Compliance evidence

---

## 🧭 Navigation

🏡 [Home](../README.md)  
📁 [Evidence Folder](evidence/README.md)  
📘 [Control Mapping](control-mapping.md)  
📗 [ISO 27001 Gap Analysis](iso27001-gap-analysis.md)  
📙 [Policy Hierarchy](policy-hierarchy.md)  
📕 [Risk Register](risk-register.md)  
⬅️ [Back to GRC Projects](README.md)  
⬆️ [Top](#)
